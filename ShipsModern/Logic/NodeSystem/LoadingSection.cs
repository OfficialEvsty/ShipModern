...
namespace ShipsForm.Logic.NodeSystem
{
    sealed class LoadingSection : IEventObserver<ShipOnLoadingState>
    {
        ...
        private List<CargoShip> m_shipsOnLoading = new List<CargoShip>();
        private Node m_node;
        ...
        private void ShipTryGetPlaceForLoading()
        {
            ...
            for (int i = 0; i < m_node.Ships.Count; i++)
            {
                ...
                {
                    CargoFraght fraght = m_node.Ships[i].Fraght;
        ...
        public bool ContractAvailableCargo(CargoFraght fraght)
        {
            ...
            foreach (Cargo key in requiredCargo.Keys)
            {
                ...
                if (fraght.Fraghter is CargoShip cargoShip)
                    cargoShip.Behavior.BoardCargo.ContractCargo();
                else throw new Exception("This ship isn't a cargo ship type.");
            }
